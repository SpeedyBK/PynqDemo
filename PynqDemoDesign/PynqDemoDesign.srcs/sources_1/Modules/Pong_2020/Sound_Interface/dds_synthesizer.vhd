...
begin
  ...
  begin
    if rst_i = '1' then
    ...
    elsif (rising_edge(clk_i) and dds_ena_i = '1') then
      ...
      if (quadrant_2_or_4 = '1' and phase(PHASE_WIDTH - 3 downto 0) = conv_std_logic_vector (0, PHASE_WIDTH - 2)) then
        ...
        if (wave_type_i = "00") then 
            lut_out <= sine_lut(conv_integer(lut_in));
        elsif (wave_type_i = "01") then 
            lut_out <= square_lut(conv_integer(lut_in));
        elsif (wave_type_i = "10") then 
            lut_out <= triangle_lut(conv_integer(lut_in));
        else
            lut_out <= sine_lut(conv_integer(lut_in));
        end if; 
      end if;
